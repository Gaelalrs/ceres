...
import core
from ceres.player import Player
from ceres.projectiles import Projectiles
...
def setup():
    ...
    core.fps = 60
    core.memory('states', States.GAME)
    core.memory('monJoueur',Player())
    core.memory('Tirer',Projectiles())



def run():
    core.cleanScreen()
    if core.memory('states') == States.GAME:
        display_game()
...
def display_game():
    core.memory('monJoueur').show()
    core.memory("monJoueur").movePlayer()
    core.memory("Tirer").shoot()
...
def text_spacequest(color, texte, position):
    myfont = pygame.font.Font("W:\\ravi2\info\ceres2\ceres\\assets\\fonts\\Spacequest.ttf", 120)
